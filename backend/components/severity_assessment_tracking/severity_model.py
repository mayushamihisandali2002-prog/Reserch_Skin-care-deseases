...
class SeverityModel:
    ...
    def predict_from_pil(self, image: Image.Image, use_tta: bool = True, disease_name: Optional[str] = None) -> dict[str, Any]:
        ...
        if model_level != score_level:
            consistency_warning = (
                "Model class and score-derived level disagreed. "
                "The score-derived level is being used as the final result."
            )
        ...
        if face_visible is False:
            quality_notes.append(
                "No face was detected. This severity model is calibrated for face images, "
                "so the result should be treated as low reliability."
            )
            confidence *= 0.5

        return {
            "severity_level": self._display_level(final_level),
