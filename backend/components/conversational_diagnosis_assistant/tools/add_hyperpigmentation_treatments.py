...
TREATMENT_CSV_PATH = (
    ...
    / "treatment_records_clean.csv"
)

def add_records():
    try:
        df = pd.read_csv(TREATMENT_CSV_PATH)
        ...
        new_df.to_csv(TREATMENT_CSV_PATH, mode="a", header=False, index=False)
        print(
            f"[Done] Added {len(new_records)} hyperpigmentation treatment records to {TREATMENT_CSV_PATH}"
        )
